...
Codecast 使用 YAML 配置文件来定义远程机器、Bot 设置和系统行为。默认读取项目根目录下的 `config.yaml`。
...
| 字段 | 类型 | 默认值 | 必填 | 说明 |
|------|------|--------|------|------|
| `install_dir` | string | `~/.codecast/daemon` | 否 | Daemon 在远程机器上的安装目录 |
| `auto_deploy` | bool | `true` | 否 | 是否自动部署 Daemon 到远程机器 |
| `log_file` | string | `~/.codecast/daemon.log` | 否 | Daemon 日志文件路径（在远程机器上） |
