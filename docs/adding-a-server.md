...
This guide covers how to add a remote machine to Codecast's `config.yaml`. Four scenarios are covered:
...
Codecast implements proxy-jump natively using asyncssh — it does not depend on your local `~/.ssh/config`.
...
> **Note:** Codecast treats machines that appear only as `proxy_jump` targets (and have no `default_paths`) as pure jump hosts — they are hidden from the `/ls machine` and `/start` autocomplete output.
...
echo "my-ssh-password" > ~/.ssh/codecast-pw
chmod 600 ~/.ssh/codecast-pw
```
...
By default, Codecast calls `node` on the remote machine. If Node.js is installed in a non-standard location (e.g. via `nvm`, `fnm`, or a custom prefix), specify the full path with `node_path:`.
...
When `node_path` is set, Codecast:
...
sudo systemctl restart codecast
```
